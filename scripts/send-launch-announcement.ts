/**
 * Script to send the launch announcement ("Tomorrow, it begins.") to subscribers
 *
 * Usage: npx tsx scripts/send-launch-announcement.ts [options] [emails...]
 *
 * Options:
 *   --dry-run    Preview what would be sent without actually sending
 *   --test       Send only to the first subscriber
 *
 * Examples:
 *   npx tsx scripts/send-launch-announcement.ts --dry-run             # Preview
 *   npx tsx scripts/send-launch-announcement.ts email@x.com           # Specific email
 *   npx tsx scripts/send-launch-announcement.ts                       # Send to ALL subscribers
 */
...
const SUBJECT = "Tomorrow, it begins.✨";
const TEMPLATE_FILE = "launch-announcement.html";
...
async function main() {
  console.log("\n🚀 Brendia Pro® Launch Announcement Sender\n");
  console.log("=====================================\n");
  ...
  const templatePath = path.join(process.cwd(), "emails", TEMPLATE_FILE);
  ...
  for (const subscriber of targetSubscribers) {
    ...
    try {
      ...
      // Rate limiting - wait 600ms between emails (Resend allows 2 req/s)
